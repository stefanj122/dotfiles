...
local widget_themes = require("ui.panels.notification-panel.github-contributions.theme")
...
local function worker(user_args)
    ...
    local theme = args.theme or 'dracula'
    ...
    local function get_square(color)
        ...
        return wibox.widget{
            ...
            draw = function(_, _, cr, _, _)
                ...
                cr:rectangle(0, 0, with_border and 4 or 5, with_border and 4 or 5)
                cr:fill()
            end,
    ...
    local update_widget2 = function(_, stdout, _, _, _)
        for _,v in pairs(stdout) do
            if day_idx %7 == 0 then
                table.insert(row, col)
                col = {layout = wibox.layout.fixed.vertical}
            end
            table.insert(col, get_square(widget_themes[theme][tonumber(v.intensity)]))
            day_idx = day_idx + 1
        end
        github_contributions_widget:setup(
            {
    ...
    github_daemon:connect_signal("contributions", function(_, data)
        print("github_daemon:connect_signal",data)
        update_widget2(github_contributions_widget, data)
    end)
