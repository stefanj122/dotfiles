...
return function(s)
	...
	-- Hiden widgets
	-- ~~~~~~~~~~~~
	local function hiden_widgets()
		s.cpu = require("ui.panels.top-panel.cpu")()
		s.ram = require("ui.panels.top-panel.ram")()
		s.temp = require("ui.panels.top-panel.temp")()
		s.keyboard = require("ui.panels.top-panel.keyboard")()
		local widget = wibox.widget({
			widget = wibox.container.constraint,
			strategy = "max",
			width = dpi(0),
		...
		local arrow = wbutton.text.state({
			...
			text = "",
			on_turn_on = function(self)
				system_tray_animation:set(900)
				self:set_text("")
			end,
			on_turn_off = function(self)
				system_tray_animation:set(0)
				self:set_text("")
				awesome.emit_signal("widget::cpu_popups:hide")
				awesome.emit_signal("widget::keyboard:hide")
			end,
		})
