...
`Async-Bytecomp-Package mode' mode.  If the prefix argument is positive, enable
...
`(default-value \\='async-bytecomp-package-mode)'.
...
This is a minor mode.  If called interactively, toggle the `Dired-Async mode'
...
To check whether the minor mode is enabled in the current buffer, evaluate
`(default-value \\='dired-async-mode)'.
